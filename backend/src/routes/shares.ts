      ...
      const existingShare = await db
        ...
        .where(
          and(
            ...
            eq(characterShares.sharedWithUserId, targetUser.id),
          ),
        )
        .limit(1)
        .get();
  ...
  fastify.delete<{ Params: ShareParams }>(
    ...
    async (request, reply) => {
      ...
      const share = await db
        .select()
        .from(characterShares)
        .where(eq(characterShares.id, shareId))
        .limit(1)
        .get();
  ...
  fastify.get<{ Params: CharacterParams }>(
    ...
    async (request, reply) => {
      ...
      const share = await db
        ...
        .from(characterShares)
        .where(
          and(eq(characterShares.characterId, id), eq(characterShares.sharedWithUserId, userId)),
        )
        .limit(1)
        .get();
