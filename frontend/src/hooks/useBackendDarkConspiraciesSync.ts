...
export const useBackendDarkConspiraciesSync = () => {
    ...
    const syncDarkConspiraciesFromBackend = useDarkConspiracyStore((state) => state.syncDarkConspiraciesFromBackend)
    ...
    useEffect(() => {
        ...
        const syncDarkConspiracies = async () => {
            try {
                ...
                const backendConspiracies: BackendDarkConspiracy[] = response.darkConspiracies.map((conspiracy) => ({
                    id: conspiracy.id,
    ...
    }, [current, isAuthenticated, isReadyToSave, saveSignature, updateCurrentDarkConspiracyIdAndVersion, userId])
}
