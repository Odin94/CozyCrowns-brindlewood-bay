...
export const hasDarkConspiracyContent = (data: DarkConspiracyData): boolean => {
    ...
    return textFields.some((field) => field.trim().length > 0) || data.layerTwoChecks.some(Boolean) || data.layerThreeChecks.some(Boolean)
}
...
export const useDarkConspiracyStore = create<DarkConspiracyState>()(
    persist(
        (set, get) => {
            const getCurrent = () => get().darkConspiracies[get().currentDarkConspiracyIndex] || getDefaultDarkConspiracyData()
            ...
            return {
                ...
                setCurrentDarkConspiracy: (index) => {
                    ...
                    set({ currentDarkConspiracyIndex: index, current: normalizeDarkConspiracy(state.darkConspiracies[index]) })
                },
                ...
                syncDarkConspiraciesFromBackend: (backendConspiracies) => {
                    ...
                    backendConspiracies.forEach((backendConspiracy) => {
                        const existingIndex = darkConspiracies.findIndex((conspiracy) => conspiracy.id === backendConspiracy.id)
                    ...
                    const currentDarkConspiracyIndex = Math.min(state.currentDarkConspiracyIndex, darkConspiracies.length - 1)
                    set({
                        darkConspiracies,
                        currentDarkConspiracyIndex,
                        current: normalizeDarkConspiracy(darkConspiracies[currentDarkConspiracyIndex] || getDefaultDarkConspiracyData()),
                    })
                },
            }
        },
